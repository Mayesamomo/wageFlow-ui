...
import { deleteInvoice, getInvoicesByUser } from '../../redux/actions/invoice'; 
            ...
            <TableBody>
              {(rowsPerPage > 0
                ...
                <TableRow key={row._id} style={{ cursor: 'pointer' }} >
                  ...
                  <TableCell style={tableStyle} onClick={() => openInvoice(row._id)} > {row.client.company} </TableCell>
                  <TableCell style={tableStyle} onClick={() => openInvoice(row._id)} > {moment(row.date).fromNow()} </TableCell>
                  <TableCell style={tableStyle} onClick={() => openInvoice(row._id)} > {row.totalAmount ? toCommas(row.totalAmount) : row.totalAmount} </TableCell>
                  ...
                  <TableCell style={{ ...tableStyle, width: '10px' }}>
                    <IconButton onClick={() => dispatch(deleteInvoice(row._id, toast))}>
